...
const TodoCard: FC<TodoCardType> = ({todoItem,onStatusChange,onEdit,onDelete,}) => {
  ...
  return (
    <>
     <div
        className={` max-w-sm w-full   sm:p-4  py-6 px-2  shadow-lg   dark:text-[#9fb9d0] dark:bg-[#232d35] bg-gray-200  } `}  style={{ borderRadius: "20px" }}  >
        <div className="flex justify-between mb-4 items-center">
          <div className="flex items-center">
            <button    type="button"  onClick={handleStatusChange}   className={`py-1 px-3.5 text-sm focus:outline-none rounded-full border  bg-[#1b2431] hover:bg-[#1b1111]   ${    todoItem.status == "COMPLETED"   ? "text-[#addf92]   border-[#addf92]   "    : "   text-[#e6b56c]  border-[#e6b56c]  "  }`}  >
          ...
          <div className={`py-1 px-3.5 text-sm rounded-full border    ${getPriorityClasses()}`} >
            {todoItem.priority}{" "}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {todoItem.labels.length > 0 &&
            todoItem.labels.map((label) => ( <div className={`py-1 px-3.5 text-sm rounded-full border text-[#9fb9d0] bg-[#232d35] `}  key={label}    >
                {label}
